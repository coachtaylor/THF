...
describe('Rules Engine', () => {
  describe('Binding Safety Rules (BS-01 to BS-05)', () => {
    it('BS-01: excludes exercises unsafe for binding users', async () => {
      ...
      const exercises = [
        createMockExercise({ id: '1', name: 'Safe Exercise', binder_aware: true, heavy_binding_safe: true }),
        createMockExercise({ id: '2', name: 'Unsafe Exercise', binder_aware: false, heavy_binding_safe: false }),
      ];

      const result = await evaluateSafetyRules(bindingProfile, exercises);

      // Should have binding safety rule applied
      expect(result.rules_applied.some(r => r.rule_id === 'BS-01')).toBe(true);
    });
    ...
    it('BS-02: applies volume reduction for 8+ hour binding', async () => {
      ...
      const bs02Applied = result.rules_applied.find(r => r.rule_id === 'BS-02');
      expect(bs02Applied).toBeDefined();
      // BS-04 also triggers (10+ hours), so modifications are accumulated
      expect(result.modified_parameters.volume_reduction_percent).toBeGreaterThanOrEqual(25);
      expect(result.modified_parameters.rest_seconds_increase).toBeGreaterThanOrEqual(30);
    });
    ...
    it('BS-03: injects binder break checkpoints for binding users', async () => {
      ...
      const binderBreak = result.required_checkpoints.find(c => c.type === 'binder_break');
    ...
    it('BS-04: reduces overhead movement volume for 10+ hour binding or ace bandage', async () => {
      ...
      const bs04Applied = result.rules_applied.find(r => r.rule_id === 'BS-04');
    ...
    it('BS-05: injects post-workout binder removal reminder', async () => {
      ...
      const postWorkoutReminder = result.required_checkpoints.find(c => c.type === 'post_workout_reminder');
    ...
    it('does NOT apply binding rules to non-binding users', async () => {
      ...
      const bindingRules = result.rules_applied.filter(r => r.rule_id.startsWith('BS-'));
  ...
  describe('Post-Operative Rules (PO-01 to PO-24)', () => {
    ...
    it('does NOT apply post-op rules to users without surgeries', async () => {
      ...
      const postOpRules = result.rules_applied.filter(r => r.rule_id.startsWith('PO-'));
  ...
  describe('HRT Adjustment Rules (HRT-01 to HRT-07)', () => {
    it('HRT-01: applies recovery multiplier for MTF on estrogen 3+ months', async () => {
      ...
      const hrtRuleApplied = result.rules_applied.find(r => r.rule_id.startsWith('HRT-'));
    ...
    it('does NOT apply HRT rules to non-HRT users', async () => {
      ...
      const hrtRules = result.rules_applied.filter(r => r.rule_id.startsWith('HRT-'));
  ...
  describe('Dysphoria Filtering Rules (DYS-01 to DYS-05)', () => {
    it('DYS-01: applies soft filter for chest dysphoria', async () => {
      ...
      const dys01Applied = result.rules_applied.find(r => r.rule_id === 'DYS-01');
    ...
    it('DYS-02: excludes mirror-required exercises for mirror dysphoria', async () => {
      ...
      const dys02Applied = result.rules_applied.find(r => r.rule_id === 'DYS-02');
    ...
    it('DYS-03: excludes partner-required exercises for body contact dysphoria', async () => {
      ...
      const dys03Applied = result.rules_applied.find(r => r.rule_id === 'DYS-03');
    ...
    it('applies multiple dysphoria rules for users with multiple triggers', async () => {
      ...
      const dysphoriaRules = result.rules_applied.filter(r => r.rule_id.startsWith('DYS-'));
