...
let _analyticsDb: SQLite.SQLiteDatabase | null = null;

function getDb(): SQLite.SQLiteDatabase {
  if (!_analyticsDb) {
    _analyticsDb = SQLite.openDatabaseSync("transfitness.db");
  }
  return _analyticsDb;
}
 ...
 */
export async function trackWhyThisWorkoutOpened(
  workoutId?: string,
): Promise<void> {
...
export async function getEventCount(
  ...
  try {
    ...
    const stmt = getDb().prepareSync(query);
    const result = stmt.executeSync(params).getAllSync() as Array<{
      count: number;
    }>;
    stmt.finalizeSync();
...
export async function getAnalyticsSummary(): Promise<{
  ...
  try {
    const [generated, started, completed, whyOpened, guides] =
      await Promise.all([
...
export async function clearAnalytics(): Promise<void> {
  try {
    getDb().execSync("DELETE FROM analytics_events;");
