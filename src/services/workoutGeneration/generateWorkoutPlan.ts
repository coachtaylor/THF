...
import {
  injectSafetyCheckpoints,
  convertToPrescriptions,
} from "./checkpointInjection";
...
export async function generateWorkoutPlan(
  ...
  logger.log(
    `   ✓ Excluded ${safetyContext.excluded_exercise_ids.length} unsafe exercises`,
  );
  logger.log(`   ✓ Critical blocks: ${safetyContext.critical_blocks.length}`);
  logger.log(
    `   ✓ Required checkpoints: ${safetyContext.required_checkpoints.length}`,
  );
  ...
  const { exercises: filteredExercises } =
    await getFilteredExercisePool(profile);
  logger.log(`   ✓ Available exercises: ${filteredExercises.length}`);

  // VALIDATION: Ensure minimum exercise count
  const MIN_EXERCISES_REQUIRED = 5;
  if (filteredExercises.length < MIN_EXERCISES_REQUIRED) {
    const errorMessage =
      filteredExercises.length === 0
        ? "No exercises available after applying safety filters."
        : `Only ${filteredExercises.length} exercises available. At least ${MIN_EXERCISES_REQUIRED} exercises are required.`;

    logger.error(`   ❌ ${errorMessage}`);
    logger.error(
      `   Applied rules: ${safetyContext.rules_applied.map((r) => r.rule_id).join(", ")}`,
    );

    throw new Error(
      `INSUFFICIENT_EXERCISES: ${filteredExercises.length} available.\n` +
        `Applied safety rules: ${safetyContext.rules_applied.map((r) => r.rule_id).join(", ")}`,
    );
  }
  ...
  for (let dayNumber = 1; dayNumber <= daysCount; dayNumber++) {
    ...
    const maxWorkoutMinutes =
      safetyContext.modified_parameters.max_workout_minutes;
    ...
    for (const duration of [30, 45, 60, 90] as const) {
      ...
      if (maxWorkoutMinutes && duration > maxWorkoutMinutes) {
        logger.log(
          `     ├─ ${duration}min variant - ⚠️ SKIPPED (exceeds ${maxWorkoutMinutes}min safety limit for binding)`,
        );
      ...
      try {
        ...
        const estimatedMainWorkoutDuration =
        ...
        logger.log(
          `     ✓ ${duration}min: ${workout.exercises.length} exercises`,
        );

        // Save individual workout to database (optional, can be done in batch later)
        try {
          await saveWorkoutToDatabase(assembledWorkout, profile, safetyContext);
        } catch (error) {
          console.warn(
            `     ⚠️  Database save skipped for ${duration}min variant`,
          );
        }
      } catch (error) {
        console.error(
          `     ❌ Failed to generate ${duration}min variant:`,
          error,
        );
  ...
  const goals = (profile.goals as string[] | undefined) || [
  ...
  logger.log(
    `   ✓ Total workouts: ${plan.days.length * 4} (4 variants per day)`,
  );
  ...
  logger.log(
    `   • Safety Rules Applied: ${safetyContext.rules_applied.length}`,
  );
  logger.log(
    `   • Exercises Excluded: ${safetyContext.excluded_exercise_ids.length}`,
  );
 ...
 */
function determineWeightGuidance(exercise: Exercise, profile: Profile): string {
 ...
 */
function estimatePrescriptionsDuration(
